...
public class OrderController {
    ...
    public ModelAndView submitOrder(String receiveAddressId, BigDecimal totalAmount, String tradeCode, HttpServletRequest request, HttpServletResponse response, ModelMap modelMap){
        ...
        if (success.equals("success")){
            ...
            //omsOrder.setFreightAmount(); //运费，支付之后在省城物流信息时
            omsOrder.setMemberId(memberId);
            omsOrder.setReceiverName(nickname);
            omsOrder.setNote("快点发货");

            String outTradeNo = "gmall";
            outTradeNo = outTradeNo + System.currentTimeMillis(); //将毫秒时间戳坪街道外部订单号
            SimpleDateFormat sdf = new SimpleDateFormat("YYYYMMDDHHmmsss");
            outTradeNo = outTradeNo + sdf.format(new Date());//将字符串拼接到外部订单号
            ...
            for (OmsCartItem omsCartItem : omsCartItems) {
                if (omsCartItem.getIsChecked().equals("1")){
                    ...
                    omsOrderItem.setProductSkuId(omsCartItem.getProductId());
            ...
            //重定向到支付系统
            ModelAndView modelAndView = new ModelAndView("redirect:http://payment.gmall.com/index");
            //为安全型考虑不应该传递一下参数
